...
#include <omp.h>
 ...
 Rcpp::List kalman_filter_cpp(Rcpp::List& ssm, const arma::mat& yt, 
   ...
   if(smooth == true){
     int t = B_tt.n_cols - 1;
     arma::mat Ptt_x_Ft_x_PttInv(P_tt.n_rows, P_tt.n_cols);
     
     for(int i = t - 1; i >= 0; i--){ 
       Ptt_x_Ft_x_PttInv = P_tt.slice(i) * Fm_t * gen_inv(P_tl.slice(i + 1));
       
       B_tt.col(i) = B_tt.col(i) + Ptt_x_Ft_x_PttInv * (B_tt.col(i + 1) - B_tl.col(i + 1));
       P_tt.slice(i) = P_tt.slice(i) + Ptt_x_Ft_x_PttInv * (P_tt.slice(i + 1) - P_tl.slice(i + 1)) * Ptt_x_Ft_x_PttInv.t();
 ...
 Rcpp::List kalman_filter_tvp_cpp(Rcpp::List& ssm, const arma::mat& yt, 
   ...
   if(smooth == true){
     ...
     for(int i = t - 1; i >= 0; i--){ 
       Ptt_x_Ft_x_PttInv = P_tt.slice(i) * Fm.slice(i).t() * gen_inv(P_tl.slice(i + 1));
